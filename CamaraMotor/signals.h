#ifndef CM_UTILS_SIGNALS_H
#define CM_UTILS_SIGNALS_H

#define CM_UTILS_NS_BEGIN	namespace cm { namespace utils {
#define CM_UTILS_NS_END		}}
...
#define CM_UTILS_NEW(arg) ::operator new(arg)
#define CM_UTILS_DELETE(arg) ::operator delete(arg)

// TEMPLATE CLASS signal
template<typename _ResT, typename ... _ArgTs>
class signal;

#ifndef CM_UTILS_SIGNALS_LITE
...
class signal<_ResT(_ArgTs...)> final
{
    ...
    enum class sync_stage : char
    {
        sync_stage_1 = 1, sync_stage_2 = 2
    };
    ...
    class storage final
    {
        ...
        void expand()
        {
            connection * new_block = reinterpret_cast<connection*>
                                     (CM_UTILS_NEW(sizeof(connection) * m_capacity +
                                                   sizeof(connection*)));
        ...
        void clear() noexcept
        {
            if (mp_block)
            {
                ...
                while (to_delete)
                {
                    ...
                    CM_UTILS_DELETE(to_delete);
                    to_delete = block;
                }

                CM_UTILS_DELETE(mp_block);
            }
        }
    ...
    public:
        ...
        {
            mp_block = reinterpret_cast<connection*>(CM_UTILS_NEW(m_capacity
...
class signal<_ResT(_ArgTs...)> final
{
    ...
    void expand_storage()
    {
        ...
        connection * new_block = reinterpret_cast<connection*>
                                 (CM_UTILS_NEW(new_capacity * sizeof(connection)));
...
public:
            ...
            m_blocked(false)
    {
        mp_block = reinterpret_cast<connection*>
                   (CM_UTILS_NEW(m_capacity * sizeof(connection)));
...
{
    _Ty * address = static_cast<_Ty*>(CM_UTILS_NEW(sizeof(_Ty)));
